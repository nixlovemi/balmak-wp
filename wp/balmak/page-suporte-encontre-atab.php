...
<?php
/*function filtra_atab_cidade2($arr_info, $estado){
	if($estado == ""){
		return $arr_info;
	}
...
$sql_filtro = ($V_FILTER_ESTADO != "") ? " AND ea_estado = '$V_FILTER_ESTADO' ": "";
$sql = "SELECT ea_id
								,ea_nome_empresa
								,ea_rating
								,ea_endereco
				...
				WHERE 1=1
				$sql_filtro
				ORDER BY ea_cidade, ea_nome_empresa";
...
foreach ($conn as $rs){
	...
	$v_ea_especialista = $rs->ea_especialista;
	
	if(!array_key_exists($v_cidade, $arr_info_atab)){
		$arr_info_atab[$v_cidade] = array();
	}

	array_push($arr_info_atab[$v_cidade], array(
		"nome"=>$v_nome,
		"rating"=>$v_rating,
		"endereco"=>$v_endereco,
		"numero"=>$v_numero,
		...
		"especialista"=>$v_ea_especialista,
	));
}
...
<div class="content">
  <div class="main-width">
    ...
    <?php
		// $arr_loop = filtra_atab_cidade2($arr_info_atab, $V_FILTER_ESTADO);
      ...
      <?php
			...
			foreach($arr_loop as $cidade => $emp_atab){
				echo "<div style='width: 100%; margin-left: 3%;' class='box-onde-estamos mb40'>";
				echo "  <h3 class='mb20'>$cidade</h3>";
				
				foreach($emp_atab as $atab){
					$v_nome = $atab["nome"];
					$v_rating = str_replace(",", ".", $atab["rating"]);
					$v_endereco = $atab["endereco"];
					$v_numero = $atab["numero"];
					$v_bairro = $atab["bairro"];
					$v_cidade = $atab["cidade"];
					$v_estado = $atab["estado"];
					$v_cep = $atab["cep"];
	
					$v_arr_tel = array();
					if($atab["telefone1"] != ""){
						array_push($v_arr_tel, $atab["telefone1"]);
					}
					if($atab["telefone2"] != ""){
						array_push($v_arr_tel, $atab["telefone2"]);
					}
					if($atab["telefone3"] != ""){
						array_push($v_arr_tel, $atab["telefone3"]);
					}
					$v_str_tels = implode(" / ", $v_arr_tel);
	
					$v_email = $atab["email"];
					// $v_str_ramos = implode(", ", $emp_atab["ramos"]);
					$v_str_ramos = $atab["ramos"];
					$v_img_avancado = ($atab["atab_avanc"] == 1) ? "<img width='100' src='".esc_url( get_template_directory_uri() )."/images/img-ctt-selo.jpg' class='certificado' />": "";
					$v_img_especialista = ($atab["especialista"] == 1) ? "<img width='100' src='".esc_url( get_template_directory_uri() )."/images/img-ctt-selo-especialista.jpg' class='certificado' />": "";
					
					$wdth_dv_img = 0;
					if($v_img_avancado != ""){
						$wdth_dv_img += 120;
					}
					if($v_img_especialista != ""){
						$wdth_dv_img += 120;
					}
	
					echo "<div class='item-lista-encontre-atab'>
									<div class='title'>$v_nome</div>
									<div style='' class='rating mb5'><img src='". esc_url( get_template_directory_uri() )."/images/stars-$v_rating.jpg' /></div>
									...
									<div class='dv-hold-img-selos' style='width: ".$wdth_dv_img."px;'>
										$v_img_avancado
										$v_img_especialista
									</div>
